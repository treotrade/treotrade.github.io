...
Animate.prototype.options = {
  ...
  distance: 25, // Distance to move points
...
Animate.prototype.init = function(){
  ...
  this.target = {
    position: {
      x: this.width * 0.618,// / 2,
      y: this.height * 0.618, // / 2
    }
  };
  ...
  for(var x = 0; x < this.width; x = x + this.width / this.options.density) {
    for(var y = 0; y < this.height; y = y + this.height/ this.options.density) {
      ...
      var point = new Point({
        x: x + a * Math.cos(b) * this.width/this.options.density,
        y: y + b * Math.sin(b) * this.height/this.options.density
...
Animate.prototype.animate = function() {
  ...
  for(i in this.points) {
    if(this.points[i].distanceTo(this.target, true) < 5000) {
       ...
       this.points[i].circle.opacity = 0.8;
       ...
       this.points[i].opacity = 0.04;
       this.points[i].circle.opacity = 0.2;
    } else {
       this.points[i].opacity = 0.0001;
...
Animate.prototype.mousemove = function(event) {
  if(event.pageX || event.pageY) {
    ...
    var canvasMetrics = {width: $('#canvas').width(), height: $('#canvas').height()};
    var clientX = event.pageX - canvas.left;
    // clientX = clientX * $(window).width() / canvasMetrics.width;
    var clientY = event.pageY - canvas.top;
    // clientY = clientY * $(window).height() / canvasMetrics.height;

    // console.log(clientX, clientY)
    if (clientX < 0 || clientY < 0 || clientX > canvasMetrics.width || clientY > canvasMetrics.height)
      return;

    this.target.position.x = clientX;
    this.target.position.y = clientY;
  } else if(event.clientX || event.clientY) {
    this.target.position.x = event.clientX - event.src + document.body.scrollLeft + document.documentElement.scrollLeft;
